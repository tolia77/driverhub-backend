...
class ConnectionManager:
    ...
    async def send_to_driver(self, driver_id: int, message: str):
        ...
        if driver_ws:
            await driver_ws.send_text(message)

    async def send_to_all_dispatchers(self, message: str):
        ...
        while True:
            ...
            if user["type"] == "dispatcher":
                if not target_driver_id:
                    await websocket.send_text("Error: driver_id is required")
                    continue
                await manager.send_to_driver(target_driver_id, f"Dispatcher {user['email']}: {text}")

            elif user["type"] == "driver":
                await manager.send_to_all_dispatchers(f"Driver {user['email']}: {text}")
